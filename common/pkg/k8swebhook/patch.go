...
func CreateCertPatcher(caCertFile string, webhookConfigName string, webhookName string, cooldownDuration time.Duration, delayedRetryTime time.Duration, sugar *zap.SugaredLogger, useV1Beta1CertPatcher bool) CertPatcher {
	if useV1Beta1CertPatcher {
		return NewV1Beta1Patcher(caCertFile, webhookConfigName, webhookName, cooldownDuration, time.Second, sugar)
	} else {
		return NewV1Patcher(caCertFile, webhookConfigName, webhookName, cooldownDuration, time.Second, sugar)
	}
}
...
func NewV1Patcher(caCertFile string, webhookConfigName string, webhookName string, cooldownDuration time.Duration, delayedRetryTime time.Duration, logger *zap.SugaredLogger) CertPatcher {
	return &V1Patcher{
...
func (p *V1Patcher) doMutatingWebhookPatch(client kubernetes.Interface, caBundle []byte) (retry bool) {
...
func (p *V1Patcher) doValidatingWebhookPatch(client kubernetes.Interface, caBundle []byte) (retry bool) {
