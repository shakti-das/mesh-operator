...
// countPatchActionsV1Beta1 counts the number of PATCH operations from fake client
func countPatchActionsV1Beta1(client *fake.Clientset) int {
	count := 0
...
// TestMutatingWebhookWithCooldownV1Beta1 verifies cooldown prevents rapid patches
func TestMutatingWebhookWithCooldownV1Beta1(t *testing.T) {
	// Setup fake time
	...
	webhookConfig := &v1beta1.MutatingWebhookConfiguration{
		ObjectMeta: metav1.ObjectMeta{
			Name: "test-mutating-config-v1beta1",
		},
	...
	patcher := &V1Beta1Patcher{
		caCertFile:        certFile,
		cooldownDuration:  5 * time.Second,
		webhookConfigName: "test-mutating-config-v1beta1",
	...
	err = patcher.PatchMutatingWebhookConfigForV1Beta1(client.AdmissionregistrationV1beta1().MutatingWebhookConfigurations(), caBundle1)
	if err != nil {
		t.Fatalf("First patch failed: %v", err)
	}

	patchCount1 := countPatchActionsV1Beta1(client)
	if patchCount1 == 0 {
	...
	err = patcher.PatchMutatingWebhookConfigForV1Beta1(client.AdmissionregistrationV1beta1().MutatingWebhookConfigurations(), caBundle2)
	if err != nil {
		t.Fatalf("Second patch after cooldown failed: %v", err)
	}

	patchCount2 := countPatchActionsV1Beta1(client)
	if patchCount2 <= patchCount1 {
		t.Fatalf("Expected more patches after cooldown expired: before=%d after=%d", patchCount1, patchCount2)
	}
}

// TestValidatingWebhookWithCooldownV1Beta1 verifies cooldown prevents rapid patches
func TestValidatingWebhookWithCooldownV1Beta1(t *testing.T) {
	// Setup fake time
	var fakeTime atomic.Value
	now := time.Now()
	fakeTime.Store(now)
	...
	webhookConfig := &v1beta1.ValidatingWebhookConfiguration{
		ObjectMeta: metav1.ObjectMeta{
			Name: "test-validating-config-v1beta1",
		},
	...
	patcher := &V1Beta1Patcher{
		...
		webhookConfigName: "test-validating-config-v1beta1",
		webhookName:       "test-webhook",
		delayedRetryTime:  1 * time.Second,
		logger:            createTestLogger(),
		timeProvider:      timeProvider,
	...
	// Manually call patch (simulating first file change)
	caBundle1, _ := os.ReadFile(certFile)
	err = patcher.PatchValidatingWebhookConfigForV1Beta1(client.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations(), caBundle1)
	if err != nil {
		t.Fatalf("First patch failed: %v", err)
	}
	...
	caBundle2, _ := os.ReadFile(certFile)
	err = patcher.PatchValidatingWebhookConfigForV1Beta1(client.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations(), caBundle2)
	if err != nil {
		t.Fatalf("Second patch after cooldown failed: %v", err)
	}
...
// TestFileSystemWatcherWithMutatingWebhookV1Beta1 tests basic patch logic
func TestFileSystemWatcherWithMutatingWebhookV1Beta1(t *testing.T) {
	...
	// Create temp CA cert file
	certFile := createTempCACertFile(t, "initial-bundle")
	...
	// We can't fully test Start methods without in-cluster config
	...
	err = patcher.PatchMutatingWebhookConfigForV1Beta1(client.AdmissionregistrationV1beta1().MutatingWebhookConfigurations(), caBundle)
	if err != nil {
	...
	for _, action := range actions {
		if action.GetVerb() == "patch" {
			...
			if patchAction.GetName() == "test-mutating-config-v1beta1" {
				foundPatch = true
				break
			}
		}
...
// TestFileSystemWatcherWithValidatingWebhookV1Beta1 tests basic patch logic
func TestFileSystemWatcherWithValidatingWebhookV1Beta1(t *testing.T) {
	// Setup fake time for cooldown control
	var fakeTime atomic.Value
	now := time.Now()
	fakeTime.Store(now)
	...
	err = patcher.PatchValidatingWebhookConfigForV1Beta1(client.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations(), caBundle)
	if err != nil {
		t.Fatalf("Patch failed: %v", err)
	}
	...
	for _, action := range actions {
		if action.GetVerb() == "patch" {
			patchAction := action.(k8stesting.PatchAction)
			if patchAction.GetName() == "test-validating-config-v1beta1" {
				foundPatch = true
				break
			}
		}
...
func TestDoMutatingWebhookPatchV1Beta1(t *testing.T) {
	webhookConfig := &v1beta1.MutatingWebhookConfiguration{
		ObjectMeta: metav1.ObjectMeta{
			Name: "test-mutating-config-v1beta1",
		},
	...
	patchCount := countPatchActionsV1Beta1(fakeClient)
	if patchCount == 0 {
		t.Fatal("Expected patch action")
	}
}

func TestDoValidatingWebhookPatchV1Beta1(t *testing.T) {
	webhookConfig := &v1beta1.ValidatingWebhookConfiguration{
		ObjectMeta: metav1.ObjectMeta{
			Name: "test-validating-config-v1beta1",
		},
